# Hey, I'm Devesh 

**BCA final year · ML + Full Stack Student · India**

![Open to remote](https://img.shields.io/badge/Open%20to-Remote-blue?style=flat-square)
![Open to freelance](https://img.shields.io/badge/Open%20to-Freelance-blue?style=flat-square)
![Open to on-site](https://img.shields.io/badge/Open%20to-On--site-blue?style=flat-square)

I build things on both ends — machine learning models that actually work, and full-stack web apps people can use. I like the part where data stops being a spreadsheet and starts being useful.

---

## What I work with

| Machine Learning | Full Stack | Tooling |
|---|---|---|
| Python, scikit-learn | React, Express.js | Git & GitHub |
| Pandas, NumPy | Node.js, MySQL | REST APIs |
| Matplotlib, Seaborn | HTML/CSS/JS | Postman, VS Code |

---

## Projects worth looking at

**Fitlife-Gym-Landing-Page(https://fit-life-gym-landing-page-bs3kksqur-devesht388-9558s-projects.vercel.app/)**
Modern fitness landing page showcasing gym services, memberships, and workout programs to help users quickly explore, join, and start their fitness journey online.

`React` `Tailwind CSS`

**[Customer Segmentation Clustering Model](https://customer-segmentation-clusteringmodel-1.onrender.com/)**
Customer segmentation model that uses K-Means clustering to group customers based on behavior and spending patterns, helping businesses target marketing strategies more effectively with data-driven insights.

`Python` `Numpy` `Pandas` `Scikit-learn`

---

## What I'm building right now

> **Human Pose Extimation Using Deep Learning** —Deep learning–based human pose estimation system that detects body keypoints from images/videos to analyze posture and movement, enabling applications like fitness tracking and real-time activity analysis.
>  Stack: Python, [Tensorflow, OpenCV, Movenet, Matplotlib].  
> Currently working on: [Real time pose detection with posture correction alert].

---

## Get in touch

If you have a project, a job, or just a question — I'm easy to reach.

📧 work.deveshtiwari388@gmail.com 
💼 [LinkedIn](https://www.linkedin.com/in/devesh-tiwari0001/)  


*Based in India · Available for remote work worldwide · Usually reply within a day*
